...
def dashboard():
    ...
    # 2. Count how many times this specific student has sat in
    used_sessions = conn.execute(
        'SELECT COUNT(*) as count FROM sitin_records WHERE id_number = ?',
        (session['student_id'],)
    ).fetchone()['count']
    
    # 3. Calculate remaining (Starting from 30)
    ...
    # Pass 'remaining' to the dashboard.html template
...
# ONLY ONE ROUTE FOR SEARCH STUDENT
